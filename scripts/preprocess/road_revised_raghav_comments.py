...
#########################################################################################
# I am not sure why you have created copies of the id, from_node, end_node columns
# This might add unecessary information and space to your DataFrame
# I have just renamed them
# Avoid loops as they add to computation time. 
# If you had a lot of road classes then this will be slow  
...
def select_partial_roads(
    ...
    selected_links = road_links[road_links[col_name].isin(list_of_values)]
...
#########################################################################################
# NEVER loop a dataframe with iterrows(). It is very slow. 
# Use itertuples() for other methods! 
# If you want to find the nearest road to each polygon geometry then simply use
# geopandas sjoin_nearest function, which is very fast
...
def find_nearest_node(
    ...
    nearest_nodes = gpd.sjoin_nearest(zones,road_nodes) # This gives nearest node to each zone
    # In case a zone is nearest to more than 1 node and you only want one
    nearest_nodes = nearest_nodes.drop_duplicates(subset=[zone_id_column],keep="first")
    nearest_node_dict = dict(zip(nearest_nodes[zone_id_column],nearest_nodes[node_id_column]))

    return nearest_node_dict
...
#########################################################################################
# I do not understand the use of this function. 
# It would seem to me that if your aim is to loop through the OD dataframe by unqiue origins
# Then all you need to do is set the "origin_node" column as the dataframe index
# You do not need to create extra datatypes like the dictionaries of destinations and flows
# Anyway I do not want to change any of your function outputs so I return the same values as you

# Again - NEVER loop a dataframe with iterrows(). It is very slow. 
# Use itertuples() for other methods! 
...
def extract_od_pairs(
    ...
    list_of_origin_nodes = list(set(od["origin_node"].values.tolist()))
    dict_of_destination_nodes: Dict[str, List[str]] = defaultdict(list)
    dict_of_origin_supplies: Dict[str, List[float]] = defaultdict(list)
    od = od.set_index("origin_node")
    for origin in list_of_origin_nodes:
        dict_of_destination_nodes[origin].append(
                                    od.loc[[origin], "destination_node"].values.tolist()
                                        )  # {nd_id: [nd_id...]}
        dict_of_origin_supplies[origin].append(
                                    od.loc[[origin], "Car21"].values.tolist()
                                    )  # {nd_id: [car21...]}
...
######################################################################################################
# There is a very straightforward way to create an igraph object from a dataframe
# Your way is very complicated and probably much slower
# Also I do not understand why you have created so many other outputs, which look like the same things
# Ideally you could just modify the road_links DataFrame and return it with an igraph network 
# I am not sure why all the dictionaries are needed when all the information is in the dataframe 
# Dictonaries can be slower than dataframes and here they will also occupy more memory space which will
# slow your computations
def create_igraph_network(
    road_links: gpd.GeoDataFrame
) -> Tuple[
    igraph.Graph,
...
######################################################################################################
# This function seems to be unnecessary if you were not working with creating dictionaries
# You can simply work with the OD dataframe and now try to create the dictionaries
# I am not sure why all the dictionaries are needed when all the information is in the dataframe 
# Dictonaries can be slower than dataframes and here they will also occupy more memory space which will
# slow your computations
...
def update_od_matrix(
    ...
    isolated_flow_matrix = temp_flow_matrix[
        temp_flow_matrix["path"].apply(lambda x: len(x) == 0)
    ]
    temp_flow_matrix = temp_flow_matrix[
        temp_flow_matrix["path"].apply(lambda x: len(x) != 0)
    ]
    return (isolated_flow_matrix,temp_flow_matrix)
...
######################################################################################################
# This function could be much faster if you were simply modifying dataframe and then converting
# it into an igraph object 
# There is a lot of repetition with the create_igraph_network()
# I am not modifyig it as it is a bit difficult to unpack
# But I will revisit it later
# I think I need ot understand the network_flow_model() first to understand how this is being used
######################################################################################################
