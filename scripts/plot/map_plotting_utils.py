...
def plot_global_basemap(ax,include_countries=None,
                        ...
                        label_size=6.0,include_scalebar=True):
    ...
    if include_countries is not None:
        boundary_gdp = boundary_gdp[boundary_gdp["ADM0_A3"].isin(include_countries)]
    ...
    if include_labels is True:
        ...
        labels = boundary_gdp[['ADM0_A3','geometry']]
        labels = labels[labels["ADM0_A3"].isin(label_countries)]
        plot_basemap_labels(ax,labels=labels,label_column="ADM0_A3",label_size=label_size)
    # plot_scale_bar(ax,scalebar_location=(0.90,0.04),scalebar_distance=100,zorder=20)
    if include_scalebar is True:
...
def plot_polygon_assets(ax,polygon,label,
                        edgecolor='white',
                        linewidth=0.4,
                        facecolor='#e0e0e0',
                        zorder=2,
                        projection=ccrs.PlateCarree()):
    for boundary in polygon.itertuples():
        ax.add_geometries(
                [boundary.geometry],
                crs=projection,
                edgecolor=edgecolor,
...
def plot_point_assets(ax,nodes,label,colors="blue",size=12,marker="o",zorder=3):
